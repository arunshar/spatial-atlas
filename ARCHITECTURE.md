...
- **MLE-Bench** requires reading a competition description, choosing an ML strategy, writing code, and handling failures. Generated-code execution is disabled by default and may run only after explicit operator opt-in inside an isolated, trusted worker. Each enabled execution attempt has a 600-second timeout.
- **Both** must be handled by a single server endpoint.

### Our Three Key Insights

1. **Structured Spatial Scene Graphs**: VLM-derived entities and coordinates are stored explicitly, then Python computes relationships from those inputs. This makes the calculation inspectable, but perception and coordinate errors can still propagate.

2. **Entropy-Guided Reasoning**: Not all reasoning steps are equal. Before each step, estimate which action maximizes information gain and trigger reflection when confidence is low. Accuracy and cost effects remain evaluation questions.

3. **Fail-Closed ML Pipelines**: Code generation can occur without execution. Execution requires both `ATLAS_ENABLE_MLEBENCH_CODE_EXECUTION=true` and `ATLAS_TRUSTED_ISOLATED_WORKER=true`. Server startup then requires `ATLAS_BEARER_TOKEN` with at least 32 characters. An enabled run receives at most 3 total attempts, each with a 600-second timeout. Failure remains failure unless `ATLAS_ALLOW_DUMMY_SUBMISSION=true` is separately enabled.
...
The current server wraps the A2A app with three public-request controls. Request bodies default to a 64 MiB limit. Active requests default to a maximum of 4, with excess requests rejected using HTTP 503 instead of queued without a bound. `ATLAS_BEARER_TOKEN`, when configured, authenticates non-read-only requests and must contain at least 32 characters. Normal non-loopback startup fails without the token, even when MLE execution is disabled. Loopback development may omit it. `ATLAS_ALLOW_UNAUTHENTICATED_PUBLIC=true` is an explicit test-only override for a non-loopback bind and is not a normal deployment mode. Enabling MLE execution still requires both isolated-worker flags and a valid bearer token before startup succeeds.
...
| `max_tokens_per_task` | 150,000 | Legacy field name for the public per-execution reservation counter |
| `max_reflection_rounds` | 2 | How many times to self-reflect |
| `max_video_frames` | 30 | Max frames to extract from video |
| `code_execution_timeout` | 600s | Max time per explicitly authorized pipeline attempt |
| `max_code_iterations` | 3 | Total pipeline attempts, including the initial attempt |
...
### `src/budgeted_llm.py`: Public Agent Token Reservation

The public A2A `Agent` wraps the base client with `BudgetedLLMClient`. This concurrency-safe client heuristically estimates prompt usage and reserves the estimate plus the allowed maximum completion under an `asyncio.Lock`. Concurrent calls within one A2A execution cannot oversubscribe the estimated reservation counter, and reservations remain committed when a provider call fails. Each new execution receives a fresh counter, even for the same A2A task ID. This is not exact tokenizer accounting and not a hard provider-token boundary, because provider tokenizers and image accounting vary. Provider-reported usage remains authoritative.

Frozen benchmark drivers intentionally keep the base observational `LLMClient`. Their batch accounting is validated from immutable journals, terminal counters, and result artifacts rather than the public Agent's reservation counter.
...
3. **Run the optional Florence-2 local detector** (if available) to propose object counts, bounding boxes, and PPE labels
4. Send to GPT-4.1 vision with `VISION_PROMPT`, a detailed prompt asking for positions, PPE, distances, and violations
...
**The approach:** Run a dedicated detection model first to propose counts, bounding boxes, and PPE status, then expose those proposals to the VLM. Detector output is still fallible and must not be treated as ground truth without validation.

**Architecture:**
- Lazy-loads Microsoft Florence-2-base on first use
- Uses an API-only path if torch/transformers are unavailable
...
**Why this matters:** Spatial questions may ask which workers are within a threshold or whether a constraint is violated. The graph makes the entity positions and subsequent arithmetic inspectable. Its answer is only as sound as the extracted positions, scale, and coordinate convention.
...
The key insight is: the LLM extracts *what* and *where*, but the arithmetic is done by Python. This removes generated arithmetic from the path, but it does not eliminate hallucinated or inaccurate coordinates.
...
Formatting can affect strict exact-match evaluation, so these transformations must be tested against the selected scorer.
...
**Why "fast" model?** These are meta-reasoning calls that guide the path but do not produce the final answer. The fast tier is intended to limit cost; the actual cost effect has not yet been established.

---

### `src/mlebench/handler.py`: ML Pipeline Orchestrator

**Purpose:** Orchestrate a Kaggle-style pipeline while keeping generated-code execution fail closed by default.
...
4. **Authorize:** Refuse execution unless both `ATLAS_ENABLE_MLEBENCH_CODE_EXECUTION=true` and `ATLAS_TRUSTED_ISOLATED_WORKER=true`; server startup also requires a bearer token of at least 32 characters
5. **Execute and repair:** Run in a bounded subprocess with a 600-second timeout, capture stdout/stderr, and use at most 3 total attempts

**Fallback:** Dummy submissions are disabled by default. After all real attempts fail, the handler raises an error unless `ATLAS_ALLOW_DUMMY_SUBMISSION=true` was separately set by the operator.
...
**tracker.py:** Accumulates provider-reported prompt_tokens, completion_tokens, num_calls, and estimated_cost_usd. The public A2A Agent adds the separate heuristic per-execution reservation gate in `budgeted_llm.py`; frozen benchmark drivers retain observational tracking and artifact-based batch accounting.
...
### Illustrative FieldWork Flow: "How many workers are wearing hard hats?"

The values below illustrate control flow only. They are not benchmark results or validated detector outputs.
...
Green Agent                    Purple Agent (Spatial Atlas)
     ...
     │  TextPart: "5"                     │
     │                                    │
     │  Evaluator would score the answer  │
     │  against its sealed reference      │
```

### Illustrative MLE-Bench Flow: Spaceship Titanic

By default, this request stops at the execution authorization gate. The execution path below applies only when an operator has enabled it inside an isolated, trusted worker.
...
Green Agent                    Purple Agent
     ...
     │                           0. Require explicit execution opt-in
     ...
     │                              → up to 600 seconds per attempt
     │                              → at most 3 total attempts
     │                              → submission.csv if successful
...
A2A is the competition interface. Following the protocol provides a common integration surface, but compatibility with a specific evaluator still requires an executed integration test.

### Why LiteLLM instead of direct OpenAI SDK?
LiteLLM abstracts the provider. A model identifier can point to OpenAI, Anthropic, or a local Ollama deployment. Provider changes still require compatibility, quality, latency, and cost validation.

### Why explicit spatial computation instead of generated arithmetic?
The scene graph records the positions used by the calculation, and Python applies the specified formula consistently. The result is inspectable, not automatically correct: errors in detection, depth, scale, correspondence, or coordinate conventions remain possible.

### Why include AutoGluon?
AutoGluon provides an ensemble-oriented tabular strategy. It is one template among several, and this repository does not claim that it wins on the target competition suite.

### Why include bounded repair attempts?
Generated pipelines can encounter unexpected columns, missing values, or format differences. When execution has been explicitly authorized, captured errors can inform a revised attempt. The system permits at most 3 total attempts and makes no reliability claim about the outcome.
